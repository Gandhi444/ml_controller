...
#include "ml_controller/util/marker_helper.hpp"
  ...
  const geometry_msgs::msg::Point & target, const geometry_msgs::msg::Pose & current_pose)
{
  ...
  const double radius = ml_controller::planning_utils::calcRadius(target, current_pose);
  ...
  for (double theta = -theta_range; theta <= theta_range; theta += step_rad) {
    ...
    trajectory_circle.push_back(
      ml_controller::planning_utils::transformToAbsoluteCoordinate2D(p, current_pose));
  }
