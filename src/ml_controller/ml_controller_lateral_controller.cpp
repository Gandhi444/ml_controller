...
{
  ml_controller_ = std::make_unique<MlController>();
...
double PurePursuitLateralController::calcCurvature(const size_t closest_idx)
{
  ...
  try {
    ...
    RCLCPP_WARN(rclcpp::get_logger("ml_controller"), "%s", e.what());
  ...
  autoware_auto_planning_msgs::msg::Trajectory & u)
{
  ...
  for (int64_t i = 0; i < static_cast<int64_t>(u.points.size()); ++i) {
    ...
    p.pose.orientation = ml_controller::planning_utils::getQuaternionFromYaw(yaw / count);
  }
  ...
  for (int i = 0; i < num_of_iteration; i++) {
    if (i == 0) {
      ...
      if (pp_output) {
        ...
        RCLCPP_WARN_THROTTLE(logger_, *clock_, 5000, "failed to solve ml_controller for prediction");
...
AckermannLateralCommand PurePursuitLateralController::generateOutputControlCmd()
{
  ...
  if (pp_output) {
    ...
    RCLCPP_WARN_THROTTLE(
      logger_, *clock_, 5000, "failed to solve ml_controller for control command calculation");
    if (prev_cmd_) {
  ...
  bool is_control_output, geometry_msgs::msg::Pose pose)
{
  ...
  ml_controller_->setCurrentPose(pose);
  ml_controller_->setWaypoints(planning_utils::extractPoses(*trajectory_resampled_));
